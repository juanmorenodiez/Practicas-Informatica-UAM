...
import numpy as np
import os
from perceptron import predicciones_a_fichero, compare_lists
import matplotlib.pyplot as plt

def obtener_grafica_ecm(list_epocas, list_ecms):
    plt.title("ECM por época para Adaline")
...
def adaline_entrenamiento(x_train, y_train, num_epocas=100, constante_aprendizaje = 0.1, tolerancia = 0.1, grafica=False):
    ...
    for index_salidas in range(y_train.shape[1]):
        output_layer.añadir(Neurona(umbral=0, tipo="Adaline"))  
  
    # conectamos capa de entrada a la capa de salida
    # inicalizando pesos a valores aleatorios pequeños
    input_layer.conectar_capa(output_layer, 0, 1)

    # añadimos capas a la red neuronal
    adaline.añadir(input_layer)
    adaline.añadir(output_layer) 
    ...
    while(contador_epocas < num_epocas):
        ...
        for fila_train in x_train:
            ...
            # valores a la salida de la red sin calcular activacion
            respuestas_salida = [neurona.valor_entrada for neurona in output_layer.neuronas]
            ...
            for index_salidas in range(len(valores_esperados)):
                resta = respuestas_salida[index_salidas] - valores_esperados[index_salidas]
            ...
            for index, neurona in enumerate(input_layer.neuronas):
                if neurona.tipo == Tipo.SESGO.value:
                    for index1, conexion in enumerate(neurona.conexiones):
                        resta = valores_esperados[index1] - respuestas_salida[index1]
                        variacion_sesgo = conexion.peso + constante_aprendizaje*resta

                        # sacamos la variacion de sesgo maxima
                        if abs(variacion_sesgo-conexion.peso) > valor_maximo:
                            valor_maximo = abs(variacion_sesgo-conexion.peso) 
                        conexion.peso = variacion_sesgo  
                        lista_pesos.append(conexion.peso)  
                else:
                    for index2, conexion in enumerate(neurona.conexiones):
                        resta = valores_esperados[index2] - respuestas_salida[index2]
                        variacion_peso = conexion.peso + constante_aprendizaje * resta * x_train[j][index]
                
                        # sacamos la variacion de peso maxima
                        if abs(variacion_peso-conexion.peso) > valor_maximo:
                            valor_maximo = abs(variacion_peso-conexion.peso) 
                        conexion.peso = variacion_peso    
                        lista_pesos.append(conexion.peso)
            j += 1  
            # metemos en esta matriz los pesos obtenidos en esa epoca  
        ...
        # condicion de parada de la fase de entrenamiento en ADALINE
        if valor_maximo < tolerancia:
            break
        contador_epocas += 1 
...
def adaline_clasificacion(x_test, y_test, pesos_entrenamiento, predicciones=False):
    ...
    for fila_test in x_test:
        ...
        if predicciones == True:
            predicciones_a_fichero(fila_test, valores_salida, "predicciones_adaline.txt")
