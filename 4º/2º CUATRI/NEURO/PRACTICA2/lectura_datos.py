...
def datos_entrada_salida(nombre_fichero):
    file = open("data/" + nombre_fichero, "r")
    ...
    # convertimos las salidas a bipolar
    for index, fila in enumerate(filas_fichero):
        fila_float = list(map(float, fila.split()))
        for i in range(len(fila_float)):
            if i >= numero_entradas:
                if fila_float[i] == 0.0 or fila_float[i] == 0:
                    fila_float[i] = -1.0           
        all_data_float.append(fila_float)          
