...
Node* graph_findDeepSearch (Graph *g, int from_id, int to_id){
	...
	ps = stack_ini((P_stack_ele_destroy)node_destroy, (P_stack_ele_copy)node_copy, (P_stack_ele_print)node_print);
	...
	while(stack_isEmpty(ps) == FALSE && encontrado == FALSE){
		u = stack_pop(ps);
		if(node_getEtq(u) == BLANCO){
			node_setEtq(u, NEGRO);
