...
int main(void) {
    ...
    if (pid_gestor == 0){
      ...
      for(i = 0; i < NUM_PROC; i++){             /*Gestor crea NUM_PROC hijos (participantes)*/
        ...
        if(pid <  0){
          ...
          pause();                                /*Se quedan esperando a que el padre les avise*/
      ...
      kill (pid_padre, SIGUSR2);  /*Todos los hijos están listos. El gestor avisa al padre.*/
      pause();
    ...
    pause();                      /*Sale del pause cuando el gestor avisa de que participantes listos*/
