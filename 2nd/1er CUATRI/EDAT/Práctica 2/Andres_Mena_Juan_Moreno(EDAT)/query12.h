...
int query12(char *username1, char *username2) {
    SQLHENV env;
    SQLHDBC dbc;
    SQLHSTMT stmt;
    ...
    if (!SQL_SUCCEEDED(ret = SQLFetch(stmt))) {
      printf ("User %s was not following user %s\n", username1, username2);
      return 0;
    }
    ...
    sprintf (query3, "delete from follows where userfollower=%lld and userfolloweed=%lld", userid1, userid2);
